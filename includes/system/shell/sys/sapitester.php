...
class SapitesterSystool extends TooBasic\Shell\ShellTool {
	...
	protected function setOptions() {
		...
		$this->_options->addOption(Option::EasyFactory(self::OPTION_CALL, ['--call', '-c'], Option::TYPE_NO_VALUE, $text));

//		$text = "TODO help text for: '--check', '-C'.";
//		$this->_options->addOption(Option::EasyFactory(self::OPTION_CHECK, ['--check', '-C'], Option::TYPE_VALUE, $text, 'config-name'));

		$text = "This option prompts a detailed list of available SimpleAPI configurations.";
		$this->_options->addOption(Option::EasyFactory(self::OPTION_LIST, ['--list', '-l'], Option::TYPE_NO_VALUE, $text));
		...
		$this->_options->addOption(Option::EasyFactory(self::OPTION_SPECIFIC, ['--specific', '-s'], Option::TYPE_NO_VALUE, $text));
	}
