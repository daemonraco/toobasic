...
allows you to remove inactive users and your tried to use it in your log-in
service.
...
* We created a new option with its help text and triggers, and added it to out
options.
...
But, How? Yes, you may not see the connection here. A shell tool assumes that
every option has a method that responds to it, if not `mainTask()` is called.
...
__users__.
Then you must create a class using that name (in capital-case) and the suffix
...
## Cron Tools
What is a _cron tool_? well, basicly is a _shell tool_ but it controls that only
one instance of a task is running at the same time.
...
## Profiles
If your site gets bigger, you'll probably end up with many _cron tools_ with a lot
...
Suppose you have three _cron tools_ you run one after another with a specific set
